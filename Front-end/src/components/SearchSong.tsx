...
const SearchModal = ({ isOpen, onClose }: SearchModalProps) => {
  const [searchTerm, setSearchTerm] = useState('Search here..'); //API not accepting empty string.
  const [showScrollButton, setShowScrollButton] = useState(false);
  
  const { data, loading, error } = useMusicApi(searchTerm); 
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="search-results">
          ...
          {data && <ClientPlayer rawResults={data.data} />}  {/* Pass raw results to ClientPlayer */}
        </div>
